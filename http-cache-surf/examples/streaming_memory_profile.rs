//! Streaming memory profiling example for surf
//!
...
use http_cache_surf::{Cache, StreamingCache};
...
async fn measure_cache_hit_memory_usage(
    ...
    if is_streaming {
        ...
        let client = Client::new().with(streaming_cache);

        // First request to populate cache
        let mut response1 = client.get(&url).await.unwrap();
        let _body1 = response1.body_bytes().await.unwrap();
        ...
        let mut response2 = client.get(&url).await.unwrap();
        ...
        let body_bytes = response2.body_bytes().await.unwrap();
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Memory Usage Analysis: Buffered vs Streaming Cache (Surf)");
    println!("=========================================================");
